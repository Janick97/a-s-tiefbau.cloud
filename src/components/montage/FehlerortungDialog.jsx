...
export default function FehlerortungDialog({ montageAuftrag, user, onClose, onReload, onOpenLeistungWizard }) {
  ...
  const handleEingemessen = async () => {
    ...
    try {
      ...
      const msg = `📍 Tiefbau offen gemeldet\nErforderlicher Tiefbau: ${tiefbauText.trim()}\nGemeldet von: ${userName}`;
      const currentAuftrag = await MontageAuftrag.get(montageAuftragId);
      const history = Array.isArray(currentAuftrag?.tiefbau_offen_history) ? currentAuftrag.tiefbau_offen_history : [];
      await Promise.all([
        sendChatMessage(projectId, msg, userName),
        MontageAuftrag.update(montageAuftragId, {
          tiefbau_offen: true,
          tiefbau_offen_date: now,
          tiefbau_offen_history: [...history, { date: now, user: userName, text: tiefbauText.trim() }]
        })
      ]);
      onReload && onReload();
      onClose();
      ...
      <motion.div
        ...
        <div className="bg-slate-400 px-4 py-3 flex items-center justify-between border-b">
          <div className="flex items-center gap-2">
            ...
            <Search className="text-slate-50 w-4 h-4" />
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 'start' && (
              <motion.div key="start" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-3">
                ...
                <button onClick={() => setStep('eingemessen')} className="bg-gray-100 p-4 text-left rounded-xl w-full border-2 border-blue-200 hover:bg-blue-100 transition-all">
                ...
                <button onClick={() => setStep('nachgemessen_ergebnis')} className="bg-slate-300 p-4 text-left rounded-xl w-full border-2 border-purple-200 hover:bg-purple-100 transition-all">
            ...
            {step === 'nachgemessen_ergebnis' && (
              <motion.div key="nachgemessen" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-3">
                ...
                <button onClick={handleStorungBehoben} disabled={isSaving} className="w-full p-3.5 rounded-xl border-2 border-green-200 bg-green-50 hover:bg-green-100 transition-all text-left flex items-center gap-3">
                ...
                <button onClick={() => { setKabelVon(''); setKabelBis(''); setStep('kabel_detail'); }} disabled={isSaving} className="w-full p-3.5 rounded-xl border-2 border-red-200 bg-red-50 hover:bg-red-100 transition-all text-left flex items-center gap-3">
                ...
                <button onClick={() => { setMuffeEins(''); setMuffeZwei(''); setStep('muffle_detail'); }} disabled={isSaving} className="w-full p-3.5 rounded-xl border-2 border-orange-200 bg-orange-50 hover:bg-orange-100 transition-all text-left flex items-center gap-3">
                ...
                <button onClick={() => { setHausanschlussHaus(''); setStep('hausanschluss_detail'); }} disabled={isSaving} className="w-full p-3.5 rounded-xl border-2 border-blue-200 bg-blue-50 hover:bg-blue-100 transition-all text-left flex items-center gap-3">
              ...
              <motion.div key="kabel_detail" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <Input value={kabelVon} onChange={(e) => setKabelVon(e.target.value)} placeholder="z.B. Muffe M12 / Hausnr. 14" className="h-10" autoFocus />
                  </div>
                  <div>
                    <Label className="text-sm font-semibold mb-1.5 block">Bis (Muffe / Hausnummer)</Label>
                    <Input value={kabelBis} onChange={(e) => setKabelBis(e.target.value)} placeholder="z.B. Muffe M13 / Hausnr. 22" className="h-10" />
                  </div>
                  <p className="text-xs text-gray-400">Diese Angaben werden automatisch im Chat gepostet.</p>
                </div>
                <Button onClick={() => handleTiefbauErforderlich('kabel')} disabled={!kabelVon.trim() || !kabelBis.trim() || isSaving} className="w-full bg-red-600 hover:bg-red-700 h-10">
                  {isSaving && <Loader2 className="w-4 h-4 animate-spin mr-2" />}
                  Im Chat posten & speichern
                </Button>
              </motion.div>
              ...
              <motion.div key="muffe_detail" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <Input value={muffeEins} onChange={(e) => setMuffeEins(e.target.value)} placeholder="z.B. Muffe M14 / VS 7" className="h-10" autoFocus />
                  </div>
                  <div>
                    <Label className="text-sm font-semibold mb-1.5 block">Muffe 2 <span className="font-normal text-gray-400">(optional)</span></Label>
                    <Input value={muffeZwei} onChange={(e) => setMuffeZwei(e.target.value)} placeholder="z.B. Muffe M15 / VS 8" className="h-10" />
                  </div>
                  <p className="text-xs text-gray-400">Diese Angaben werden automatisch im Chat gepostet.</p>
                </div>
                <Button onClick={() => handleTiefbauErforderlich('muffe')} disabled={!muffeEins.trim() || isSaving} className="w-full bg-orange-600 hover:bg-orange-700 h-10">
                  {isSaving && <Loader2 className="w-4 h-4 animate-spin mr-2" />}
                  Im Chat posten & speichern
                </Button>
              </motion.div>
              ...
              <motion.div key="hausanschluss_detail" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <Input value={hausanschlussHaus} onChange={(e) => setHausanschlussHaus(e.target.value)} placeholder="z.B. Hausnr. 45 oder VS 12" className="h-10" autoFocus />
                  </div>
            ...
            {step === 'behoben_erinnerung' && (
              <motion.div key="behoben_erinnerung" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="space-y-4">
                ...
                <Button onClick={() => { onClose(); onOpenLeistungWizard && onOpenLeistungWizard(); }} className="w-full bg-blue-600 hover:bg-blue-700 h-11 text-sm font-semibold">
                  <Plus className="w-4 h-4 mr-2" />
                  Jetzt Leistung erfassen
                </Button>
                <Button variant="outline" onClick={onClose} className="w-full h-10 text-sm">
                  Später erledigen
                </Button>
              </motion.div>
            )}
