              ...
              {filteredProjects.length === 0 ? (
                ...
                filteredProjects.map(project => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 text-sm hover:bg-orange-50 flex items-center gap-2 min-w-0 transition-colors border-b last:border-b-0 overflow-hidden ${targetProjectId === project.id ? 'bg-orange-50' : ''}`}
                  >
                    <span className="font-mono text-orange-700 font-semibold shrink-0 text-xs">{project.project_number}</span>
                    <span className="text-gray-700 truncate min-w-0 flex-1 text-xs">{project.title}</span>
