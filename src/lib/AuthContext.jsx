...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      if (session?.user) { const p = await loadProfile(session.user.id); if (mounted) setProfile(p); }
      setLoading(false);
    };
    init();
  ...
  const refreshProfile = async () => { if (user) { const p = await loadProfile(user.id); setProfile(p); } };

  const value = { user, profile, loading, signIn, login: signIn, signOut, logout: signOut, refreshProfile, isAdmin: profile?.role === 'admin' };
